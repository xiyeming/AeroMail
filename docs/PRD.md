...
- 支持 **English / 简体中文** 两种界面语言，可一键切换
- 语言偏好持久化到本地 SQLite，启动时自动恢复
- 切换入口覆盖：设置页、Status Bar、Command Palette
- 后端错误/状态提示通过错误码 + 参数返回，由前端统一映射为本地化文本
- 默认语言策略：先读用户设置，无设置则检测 OS 语言，最终回退到英文

### 4.8 邮件翻译

- 支持在邮件详情页对邮件正文进行翻译
- 支持两种翻译引擎：
  - **传统翻译 API**：Google Translate / DeepL / Azure Translator / 百度 / 有道 / 腾讯 / 阿里
  - **AI 翻译**：复用 AI 助手模块的 `AiProvider`，选择模型后翻译
- 翻译结果按「原文哈希 + 目标语言 + 引擎 ID」缓存到 SQLite
- 默认目标语言跟随当前界面语言：中文界面默认英，英文界面默认中；可在设置中覆盖
- 译文与原文在 MailViewer 内一键切换

### 4.9 AI 助手

- 内置国内外主流 AI 厂商预设：
  - 国际：OpenAI、Anthropic Claude、Google Gemini、Azure OpenAI
  - 国内：DeepSeek、Moonshot、通义千问、智谱 GLM、MiniMax、百川
- 默认注入当前邮件上下文（主题、发件人、正文摘要），支持总结、生成回复、提取待办
- 支持自由问答，聊天记录本地持久化
- 入口：Sidebar AI 助手面板、MailViewer 快捷按钮、Command Palette
...
| ID | 功能 | 优先级 | 说明 |
|----|------|--------|------|
| I18N-01 | 英文/简体中文切换 | P0 | 设置、Status Bar、Command Palette 均可切换 |
| I18N-02 | 语言偏好持久化 | P0 | 存储于 SQLite settings 表，启动自动恢复 |
| I18N-03 | 后端错误码本地映射 | P0 | 后端返回 `{code, args}`，前端统一翻译 |
| I18N-04 | OS 语言检测与回退 | P1 | 首次启动按系统语言自动选择 |

### 5.8 翻译模块（Translate）

| ID | 功能 | 优先级 | 说明 |
|----|------|--------|------|
| TR-01 | 传统翻译 API 配置 | P0 | 支持 Google/DeepL/Azure/百度/有道/腾讯/阿里 |
| TR-02 | AI 翻译配置 | P0 | 复用 AI 助手的 AiProvider |
| TR-03 | 邮件详情页翻译 | P0 | 原文/译文切换，目标语言可选 |
| TR-04 | 翻译结果本地缓存 | P0 | 按 source_hash + target_lang + provider_id 缓存 |
| TR-05 | 默认目标语言 | P1 | 跟随 UI 语言，可设置覆盖 |
| TR-06 | 翻译提供商测试连接 | P1 | 配置后一键测试 API 可用性 |

### 5.9 AI 助手模块（AI Assistant）

| ID | 功能 | 优先级 | 说明 |
|----|------|--------|------|
| AI-01 | 国内外主流厂商预设 | P0 | OpenAI/Claude/Gemini/Azure/DeepSeek/Moonshot/Qwen/GLM/MiniMax/Baichuan |
| AI-02 | API Key 加密存储 | P0 | AES-256-GCM 加密后存入 SQLite |
| AI-03 | 邮件上下文注入 | P0 | 默认带入当前邮件主题/正文摘要 |
| AI-04 | 快捷操作（总结/回复/待办） | P0 | MailViewer Header 一键触发 |
| AI-05 | 自由问答会话 | P0 | Sidebar 聊天面板，支持多轮对话 |
| AI-06 | 聊天记录持久化 | P1 | SQLite 保存会话与消息 |
| AI-07 | AI 提供商测试连接 | P1 | 配置后一键测试 API 可用性 |
...
| ID | 验收标准 | 测试方法 |
|----|----------|----------|
| I18N-01 | 在 Settings / Status Bar / Command Palette 切换语言后，所有界面文案在 300ms 内刷新为对应语言 | 界面切换测试 |
| I18N-02 | 删除 `app.locale` 设置并重启，应用根据 OS 语言自动选择英文或中文 | 回退测试 |
| I18N-03 | 后端返回 `IMAP_AUTH_FAILED` 错误时，前端显示对应语言的本地化提示，且包含账户名参数 | 错误映射测试 |
| I18N-04 | `en.json` 与 `zh-CN.json` 的 key 集合一致，无缺 key | CI 一致性检查 |

### 7.8 翻译（Translate）

| ID | 验收标准 | 测试方法 |
|----|----------|----------|
| TR-01 | 配置 DeepL Key 后点击测试，5 秒内返回连接成功提示 | 连接测试 |
| TR-02 | 打开一封英文邮件，点击翻译，默认目标语言为中文；再次打开同一邮件选择同一引擎，3 秒内返回缓存译文 | 缓存测试 |
| TR-03 | 切换目标语言为英文后，原文中文邮件被翻译为英文，顶部显示翻译来源横幅 | 功能切换测试 |
| TR-04 | 断网状态下，已缓存翻译的邮件仍可查看译文 | 离线测试 |
| TR-05 | 在 Settings 中修改默认目标语言后，邮件详情页翻译默认目标语言同步更新 | 设置同步测试 |

### 7.9 AI 助手（AI Assistant）

| ID | 验收标准 | 测试方法 |
|----|----------|----------|
| AI-01 | 选择 DeepSeek 预设后，base URL 和默认模型自动填充，用户只需输入 API Key | 预设填充测试 |
| AI-02 | 点击 MailViewer 的「AI 总结」按钮，5 秒内返回邮件总结文本，且 prompt 包含当前邮件主题 | 上下文注入测试 |
| AI-03 | 在 AI 助手面板发送消息后，聊天记录写入 SQLite，重启应用后可见 | 持久化测试 |
| AI-04 | 数据库中的 `api_key_encrypted` 字段为密文，无法直接读取原始 Key | 安全审计 |
| AI-05 | AI API 返回 429 时，前端显示「请求过于频繁，请稍后再试」的本地化提示 | 错误处理测试 |
