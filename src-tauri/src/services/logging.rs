...
fn build_layer(config: &LogConfig) -> (Box<dyn Layer<Registry> + Send + Sync>, WorkerGuard) {
    if config.enabled {
        let appender = RollingFileAppender::new(Rotation::DAILY, &config.dir, "aeromail.log");
