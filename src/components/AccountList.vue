...
<template>
  <div class="rounded-lg border border-border bg-card p-5">
    ...
    <div v-if="accountStore.loading" class="py-4 text-center text-sm text-muted">
      {{ $t('common.loading') }}
    </div>
    ...
    <ul v-else class="space-y-2">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div class="min-w-0">
            <div class="truncate text-sm font-medium text-text">{{ account.name }}</div>
            <div class="truncate text-xs text-muted">{{ account.email || account.imapHost }}</div>
          </div>
        </div>
        <button
